  ...
  if (orgs.length === 0) {
    return (
      <div className="space-y-6">
        ...
        {/* Camino para personas que buscan empleo (no empresas). */}
        <div className="rounded-2xl border border-slate-200 bg-white p-6 text-center dark:border-slate-800 dark:bg-slate-900">
          <p className="text-sm text-slate-600 dark:text-slate-300">
            ¿Buscas empleo inclusivo? Crea tu perfil de candidato para postular
            dentro de Incluye.
          </p>
          <Link
            href="/dashboard/perfil-candidato"
            className="mt-3 inline-block rounded-lg border border-indigo-300 px-4 py-2 text-sm font-medium text-indigo-700 hover:bg-indigo-50 dark:border-indigo-800 dark:text-indigo-300 dark:hover:bg-indigo-950/40"
          >
            Crear mi perfil de candidato
          </Link>
        </div>
      </div>
    );
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {orgs.map((m) => {
            ...
            const secondary = [
              ...
              { href: `/dashboard/org/${org.id}/postulaciones`, label: "Postulaciones" },
